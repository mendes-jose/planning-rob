...
\begin{figure}[!h]
        ...
        \end{subfigure}
        \caption{Variation of total execution time with the computation horizon ($T_c$) for differet planning horizons ($T_p$) and sampling number ($N_s$) in three obstacles scenarion.}\label{fig:ttot}
\end{figure}
...
termination constraints is proposed and studied. By identifying a stop condition 
the receding horizon approach is finished and a termination planning problem is solved.
The problem is stated as constrained optimization problem which minimizes the time for reaching
a precise goal configuration through a collision-free trajectory that secures communication
between robots.
%Future work may be 
