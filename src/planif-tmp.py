  ...
  def phi1(self, z):
    if z.shape >= (self.u_dim, self.l+1):
      return np.append(z[:,0], \
                       np.matrix(np.arctan2(z[0,1], z[1,1])), axis = 0)
    else:
      # TODO ERROR
      print 'ERROR'
      return -1
...
class Trajectory_Generation(object):
  ...
  ## Unknown parameters (defining arbitrary initial value)
    # TODO: initiate unknow parameters with a fast nonoptimal algorithm
    self.t_fin = 15.0
    self.C = np.matrix(np.zeros((self.n_ptctrl,self.mrob.u_dim)))
    self.C = np.matrix(np.random.rand(self.n_ptctrl,self.mrob.u_dim))
    # Changing some values to make the initial trajectory appears better
    # TODO remove it
    #self.C[0,:] = np.matrix('0.5 1')
    #self.C[6,:] = np.matrix('0.5 4')
    #self.C[7,:] = np.matrix('0.5 -1')
    #self.C[-1,:] = np.matrix('3 1')
    #self.C[self.n_ptctrl-1,:] = np.matrix('1 -1')
...
#                        iprint=2)
  ...
  def _gen_dtraj(self, U, deriv_order):
    ...
    print 'knots', type(self.knots)
    print 'C', type(C[:,0])    
    z = si.splev(t,(np.asarray(self.knots),np.asarray(C[:,0]),self.d-1),
        der=deriv_order)
    print type(z)
    for i in range(self.mrob.u_dim)[1:]:
      z = np.append(z, np.asarray(si.splev(t,(self.knots,np.asarray(C[:,i]),self.d-1),
          der=deriv_order)), axis=1)
          
    #z = self._comb_bsp(self.t_init, C, deriv_order)
    #for t in self._gen_time(t_fin)[1:]:
    #  z = np.append(z, self._comb_bsp(t, C, deriv_order), axis = 0)
    return z

  ## Combine b-splines
  def _comb_bsp(self, t, C, deriv_order):
    return sum([C[j]*self._bspline(t, j, self.d, deriv_order) \
        for j in range(0, self.n_ptctrl)])

  ## B-spline recursive algorithm
  # TODO: Say NO to losing time with recursive functions,
  #       use iteractive with execution stack instead!
  def _bspline(self, t, j, d, der_order):
    if der_order == 0:
      if d == 1:
        return 1 if (t >= self.knots[j] and t < self.knots[j+1]) else 0
      else:
        if (self.knots[j+d-1]-self.knots[j] == 0 or \
            self.knots[j+d]-self.knots[j+1] == 0):
          return 1
        else:
          return (t - self.knots[j])/(self.knots[j+d-1]-self.knots[j])* \
              self._bspline(t, j, d-1, der_order) + (self.knots[j+d]-t)/ \
              (self.knots[j+d]-self.knots[j+1])* \
              self._bspline(t, j+1, d-1, der_order)
    else:
      if d == 1:
        return 0
      else:
        if (self.knots[j+d-1]-self.knots[j] == 0 or \
          ...
          return (d-1)*((self._bspline(t, j, d-1, der_order-1))/ \
              (self.knots[j+d-1]-self.knots[j])- \
              (self._bspline(t, j+1, d-1, der_order-1))/ \
              (self.knots[j+d]-self.knots[j+1]))
  ...
  def _fieqcons(self, U):
    ...
    all_zl = [np.append(np.append(
        ...
        self._comb_bsp(t, C, 2).transpose(), axis = 1) for t in time]
...
time = trajc._gen_time(trajc.t_fin)
