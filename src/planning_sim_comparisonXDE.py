...
class Robot(object):
    ...
    def _ls_sa_fieqcons(self, x):
        ...
        max_acc_cons = list(itertools.chain.from_iterable(
                [[self.k_mod.acc_max[i, 0]-abs(at[i, 0]) \
                for i in range(self.k_mod.u_dim)]for at in atTp]))
    ...
    def _sa_fieqcons(self, x):
        ...
        dz = self._comb_bsp(self._mtime, C, 0).T
        for dev in range(1, self.k_mod.l+2):
            dz = np.append(dz, self._comb_bsp(self._mtime, C, dev).T, axis=0)
