...
#                axis = 0)
        ...
        self.ctrl_pts[:,0] = np.matrix(np.linspace(self.q_init[0,0],
                self.q_fin[0,0], self.n_ctrlpts)).transpose()
        self.ctrl_pts[:,1] = np.matrix(np.linspace(self.q_init[1,0],
                self.q_fin[1,0], self.n_ctrlpts)).transpose()
        ...
        for r in range(self.obst_map.shape[0]):
            ...
            self.circ = self.circ + \
                    ...
                    self.obst_map[r,2]+self.mrob.rho,color='r',ls = 'dashed',
                    fill=False)]
        ...
        print('Optimization summary: {} exit code {}'.format(
                inform['text'],
                inform['value']))
    ...
    def _obj_func(self, x):
        ...
        all_zl = [np.append(np.append(
                ...
                self._comb_bsp(t, ctrl_pts, 2).transpose(), axis = 1) \
                for t in mtime]
...
#        J = -sum(V) + dist
        ...
        for m in range(1,self.obst_map.shape[0]):
            obst_cons = np.append(
                    obst_cons,
                    np.array([(self.mrob.rho + self.obst_map[m,-1]) - \
                    LA.norm(self.obst_map[m,0:-1].transpose() - zl[:,0]) \
                    for zl in all_zl]))
...
all_zl = [np.append(np.append(
        ...
        trajc._comb_bsp(t, trajc.ctrl_pts, 2).transpose(), axis = 1) \
        for t in mtime]
