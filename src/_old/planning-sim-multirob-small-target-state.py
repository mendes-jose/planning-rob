...
class PolygonObstacle(Obstacle):
    ...
    def __init__(self, vertices):
        ...
        self.cp = PolygonObstacle._calculate_polygon_centroid(np.vstack([vertices,vertices[0]]))

        # compute max dist centroid to vertices
        self.radius = max(LA.norm(self.cp-vertices,axis=1))

        # call ancestor's __init__
        Obstacle.__init__(self, list(self.cp), self.radius)
...
class Robot(object):
    ...
    def _ls_sa_fieqcons(self, x):
        ...
        for m in self.detected_obst_idxs:
            obst_cons += [self.obst[m].pt2obst(np.squeeze(np.asarray(qt[0:2,0].T)), self.rho) for qt in qtTp]
