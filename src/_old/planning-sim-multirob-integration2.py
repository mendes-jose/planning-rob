...
class Robot(object):
    ...
    def _linspace_ctrl_pts(self, final_ctrl_pt):
        self._C[0:self._n_ctrlpts, 0] = np.array(np.linspace(self._latest_z[0, 0], \
                final_ctrl_pt[0, 0], self._n_ctrlpts)).T
        self._C[0:self._n_ctrlpts, 1] = np.array(np.linspace(self._latest_z[1, 0], \
                final_ctrl_pt[1, 0], self._n_ctrlpts)).T

    def _detect_obst(self):
        idx_list = []
        for idx in range(len(self._obst)):
            dist = self._obst[idx].detection_dist(np.squeeze(np.asarray(self._latest_z.T)))
            if dist < self._d_rho:
                idx_list += [idx]
    ...
    def _ls_sa_feqcons(self, x):
        ...
        eq_cons = list(np.squeeze(np.array(self.k_mod.phi1(dztinit)-self._latest_q)))+\
                list(np.squeeze(np.array(self.k_mod.phi1(dztfinal)-self.k_mod.q_final)))+\
                list(np.squeeze(np.array(self.k_mod.phi2(dztinit)-self._latest_u)))+\
...
#        eps = self._dist_opt_offset # m
#        goal_pt = self.k_mod.q_final[0:-1, :] - self._latest_z
...
#        if not self._final_step:
        if self._plan_state != 'ls':
            ...
            self._latest_q = self.k_mod.phi1(self._all_dz[-1][:, -1].reshape(
                    self.k_mod.l+1, self.k_mod.u_dim).T)
            self._latest_u = self.k_mod.phi2(self._all_dz[-1][:, -1].reshape(
                    self.k_mod.l+1, self.k_mod.u_dim).T)
...
#        os.system("mkdir -p ../traces/pngs/"+self._sn)
#        os.system("mkdir -p "+direc+"pngs/p"+self._sn)
        os.mkdir("../traces/pngs/"+self._sn)
...
#                fig.savefig('../traces/pngs/multirobot-path-'+str(i)+'.png')
    ...
    #                print(path[i].shape)
                ...
                fig.canvas.draw()
                ctr += 1
                fig.savefig('../traces/pngs/'+self._sn+'/multirobot-path-'+str(ctr)+'.png', bbox_inches='tight')
            for i in range(1, 8):
                fig.savefig('../traces/pngs/'+self._sn+'/multirobot-path-'+str(ctr+i)+'.png', bbox_inches='tight')
#            ax.relim()
#            ax.autoscale_view(True, True, True)
#            fig.canvas.draw()
#            fig.savefig(direc+'pngs/p'+self._sn+'/multirobot-path.png', bbox_inches='tight')
            ...
            fig_s.savefig('../traces/pngs/'+self._sn+'/multirobot-vw.png', bbox_inches='tight')
...
#            [1.0,  5.0])]          # u_max
    ...
    latest_z = [mpc.Array('d', [kine_models[i].q_init[0, 0], \
            kine_models[i].q_init[1, 0]]) for i in range(n_robots)]
    com_link = RobotMsg(done_planning, intended_path_x, intended_path_y, latest_z)
