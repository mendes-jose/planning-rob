...
#        self.ctrl_pts[1]+np.matrix([1,2])
        ...
        while maxit > 0:
            ...
            for i in nrp:
                ...
                #epsilon = c*LA.norm(z[j]-z[dists.index(sorteddists[1])])
                ...
                if internal_intersecc1 == []:
                    ...
                    continue
                elif len(internal_intersecc1) > 1: # TODO take care of multiple
                                                   # intersections along the line segment
                    done += 1
                    continue
                    ...
                    # sort control points by distance to the obst. to be avoided
                    distcp = map(lambda cp:(cp[0,0]-to_be_avoided.x)**2 + \
                            (cp[0,1]-to_be_avoided.y)**2, self.ctrl_pts[1:-1,:])
                    ...
                    ctrlpt_index1 = distcp.index(sorteddistcp[0])+1
                    ctrlpt_index2 = distcp.index(sorteddistcp[1])+1
                    ctrlpt_index3 = distcp.index(sorteddistcp[2])+1
                    ...
                    path = path/LA.norm(path) # unit path orientation vector
    
                    dp = ((to_be_avoided.cp-z_aux)*path.T)[0,0] # dot product
    
                    z_aux2x = dp*path # aux pt to projection point vector
    
                    x2to_be_avoided = (to_be_avoided.cp-z_aux) - z_aux2x # projection pt to center

                    big_radius = self.rho + to_be_avoided.radius
    
                    radius_ortho = (big_radius)*x2to_be_avoided/LA.norm(x2to_be_avoided)

                    def gaussian(x, mu, sig):
                        return np.exp()

                    no_ctrl_to_be_moved = int(
                            (2*big_radius+np.sqrt(sorteddistz[0]))/\
                            LA.norm(self.ctrl_pts[ctrlpt_index1]-\
                            self.ctrl_pts[ctrlpt_index2]))
                    # Thresholds
                    if no_ctrl_to_be_moved < 1: # never leave without moving a ctrl pt at least
                        no_ctrl_to_be_moved = 1
                    elif no_ctrl_to_be_moved > self.n_ctrlpts-2: # nb of ctrl points
                        no_ctrl_to_be_moved = self.n_ctrlpts-2
                    ...
                    pltarg += pltmoved
                    pltarg += [to_be_avoided.x, to_be_avoided.y, 'kx',
                            other.x, other.y, 'b.',
...
#            solver.setOption('epsopt', 9e-1)
    ...
    def _update_opt(self, x):
        ...
        print('{}, {}'.format(self.angspeed[0], self.angspeed[-1]))
...
class RoundObstacle(Obstacle):
    ...
    def pltCircle(self, color='k',linestyle='solid',isFill=False, offset=0.0):
        return plt.Circle(
...
#    obst_info = [([1.6742474247424743, 2.5733573357335731], 0.57277227722772273), ([0.28292829282928289, 1.9598859885988598], 0.288973897389739), ([0.57859785978597855, 3.1326432643264326], 0.44873987398739867), ([2.645244524452445, 3.0490549054905491], 0.19252925292529252), ([1.895049504950495, 1.6162916291629161], 0.38523852385238522)]

#    obst_info = [([2.2154215421542154, 2.8564656465646561], 0.2737173717371737), ([1.6522052205220521, 1.7210921092109208], 0.325022502250225), ([1.0057605760576056, 1.8263126312631262], 0.31894689468946891), ([-0    .0024802480248025116, 2.7621662166216621], 0.38154815481548154), ([1.5374337433743372, 3.1204620462046204], 0.42029702970297023), ([0.77013701370137, 2.8814581458145812], 0.1908190819081908), ([2.9196319    631963195, 2.3099909990999099], 0.52254725472547248)]
