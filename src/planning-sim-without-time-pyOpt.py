...
#    psqp = pyOpt.PSQP(pll_type='POA')

#    [fstr, xstr, inform] = slsqp(opt_prob, sens_mode='pgc')
  ...
  def _gen_dtraj(self, x, deriv_order):
    ...
    t = np.asarray(self._gen_time(self.t_fin))
    self.knots = self._gen_knots(self.t_fin)
  ...
  def _obj_func(self, x):
    ...
    # estimating time
    S = self._gen_dtraj(x,0)
    V = self._gen_dtraj(x,1)
    # Integration to find the time spent to go from z_init to z_final
    # TODO: Improve this integration
    self.t_fin=sum(LA.norm(S[ind-1,:]-S[ind,:])/LA.norm(V[ind-1,:]/2.0+V[ind,:]/2) for ind in range(1, S.shape[0]))
