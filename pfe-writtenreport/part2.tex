...
The total obstacle penetration area ($\mathrm{P}$) can be calculated as the some of the penetration area for a planning section ($\mathrm{P}_w$), that is the plan followed within a computation horizon. $P_w$ can be estimated as follows:
...
Ideally we would have $\mathrm{P}_w = 0$. A way of guarantying that would be to increase the obstacles radius computed by the robot's perception system by the maximum distance that the robot can run within the time spam $T_p/N_s$ (equivalent to $T_c/N_{T_c}$).
