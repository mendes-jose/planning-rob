...
\section{Analysis of real-time planning feasibility and total time performance}
...
We attempt nevertheless to extract some quantitative knowledge about how these parameters impact the generated solution based on several simulations run with different parameters configurations.
...
The used parameters can be seen in table~\ref{tab:optparam}. Different maximum numbers of iterations are used for different stages of the planning process.
...
Real-time feasibility in this context can be considered as having the \textit{maximum computational time} spend for planning the path sections
...
%Here though we are only interest in understanding the variation of \textit{maximum computational time}/$T_c$ with changes on $T_c$, $T_p$, $N_s$, $N_{knots}$.
...
\subsubsection{Influence of $N_s$ on penetration area}
