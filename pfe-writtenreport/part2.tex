...
solution trajectory is identical to the one estimated in \ref{step1}. For the same reason as before, goal state constraints are still left out of the NPL.
...
The flowchart shown in Figure~\ref{fig:flowchart} aims to illustrate the whole motion planning planning algorithm presented until now. One can identify the receding horizon planning as the left group of blocks from "Update" to "Solve $NLP_{b,2}(\tau_k)$". Within this group of blocks, the decision block represents the evaluation of whether Step 2 (\ref{step2}) will be performed or not based on the evaluation of possible conflicts. Analogously, the right group of blocks represent the termination planning.
...
\begin{adjustbox}{max width=\textwidth}
    \begin{tikzpicture}
		\matrix (m)[matrix of nodes, column  sep=1cm,row  sep=8mm, align=center, nodes={rectangle,draw, anchor=center} ]{
		    ...
		    |[desicion]| {Is $\mathcal{C}_{b,com}(\tau_k) \bigcup \mathcal{C}_{b,coll}(\tau_k)$ empty?} &  &|[block]| {Computation of conflict sets}\\
		    ...
		    |[block]| {Exchange}     & &   |[block]| {Sync with $\forall R_c | R_c \in \mathcal{C}_{b,com} \bigcup \mathcal{C}_{b,coll}$}\\
...
To test and evaluate the performance of our motion planning algorithm in a near real situation we used a physics simulation environment called XDE. 
...
Our preliminary results for this implementation for a scenario described in the Table~\ref{tab:xdesimin} can be seen bellow in Figure~\ref{fig:xdepath} to~\ref{fig:xdevelo}. In red we have the solution found by the algorithm while in blue we have the simulated robot's actual path, velocities and yaw.
...
\hline
$\dot{\omega}_{max}$ & $0.60\ \mathrm{rad/s^2}$\\
\hline
\end{tabular}
\end{center}
\end{table}
...
At the beginning of the implementation of the algorithm on XDE we used the optimization library NLOPT which supports the same solver SLSQP used before, requiring though the user to implement the objective function gradient and constraints Jacobian matrix computation.

Find those analytic derivatives is not trivial. Probably due to some error while calculating our fist implementation using the SLSQP solver halted after the first iteration due to round off errors. We used then the derivative-free solver COBYLA to generate the results presented.

Besides, acceleration constraints were added to the NLPs.
